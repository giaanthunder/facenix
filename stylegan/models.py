...
class G_style(keras.Model):
    def __init__(self, max_res=1024):
        ...
        self.mapping              = G_mapping(self.z_size, w_size, 2*self.num_lod)
    ...
    def call(self, z1):
        ...
        if self.is_training:
            ...
            batch_size, num_layer, w_size = w1.shape
            # z2 = tf.random.normal([batch_size, self.z_size])
            z2 = tf.random.uniform([batch_size, self.z_size])
            w2 = self.mapping(z2)
            rand = tf.random.uniform([batch_size, num_layer,1], 0., 1.)
            mix_prob = tf.ones([batch_size, num_layer,1]) * self.style_mixing_prob
            w = tf.where(rand < mix_prob, w1,w2)
        
            # Evaluate synthesis network
            img = self.synthesis(w, rand_noise=True)
        else:
...
class G_synthesis(layers.Layer):
    def __init__(self, w_size, max_lod):
        ...
        # create list of to_rgb layers 4x4 -> 1024x1024
        self.torgb_layers = []
    ...
    #     if self.alpha == 0:
    #         x = self.conv_layers[i](x, w, i, rand_noise)
    #         img_out = self.torgb_layers[i](x)
    #     else:
    #         img_out = self.torgb_layers[i-1](x)
    ...
    # def call(self, w, rand_noise=False):
    #     img_out = tf.cond(self.alpha==0,lambda: call_1(w,rand_noise),lambda: call_2(w,rand_noise))
    #     return img_out

    # # alpha = 0
    # def call_1(self, w, rand_noise=False):
    #     x = self.conv_layers[0](w, rand_noise)
    #     i = 1
    #     while i < self.lod:
    #         x = self.conv_layers[i](x, w, i, rand_noise)
    ...
    # # alpha != 0
    # def call_2(self, w, rand_noise=False):
    #     x = self.conv_layers[0](w, rand_noise)
    #     i = 1
    #     while i < self.lod:
    #         x = self.conv_layers[i](x, w, i, rand_noise)
...
class D_basic(keras.Model):
    def __init__(self, max_res=1024):
        ...
        self.lod      = max_lod
        self.alpha    = tf.Variable(0., name='D/alpha', trainable=False)
        nf              = [512, 512, 512, 512, 512, 256, 128, 64, 32, 16]
        gain            = tf.sqrt(2.)
        hi_res_lod    = 5

        self.fromrgb_layers = []
        for i in range(max_lod+1): # 4x4 -> 1024x1024
            lay = from_rgb(nf, i)
            self.fromrgb_layers.append(lay)

        self.conv_layers = [d_primary_block(nf, 0, gain=gain)] # 4x4
        for i in range(1, hi_res_lod): # 8x8 -> 64x64
            lay = d_low_res_block(nf, i, gain=gain)
            self.conv_layers.append(lay)

        for i in range(hi_res_lod, max_lod+1): # 128x128 -> 1024x1024
    ...
    #     x = self.conv_layers[self.lod](x)

    #     if self.alpha != 0.:
    #         y = downscale2d(img)
    #         y = self.fromrgb_layers[self.lod-1](y)
    ...
    # def call(self, img_in):
    #     img_out = tf.cond(self.alpha==0,lambda: self.call_1(img_in),lambda: self.call_2(img_in))
    #     return img_out

    # def call_1(self, img_in):
    #     img = to_nchw(img_in)
    #     x = self.fromrgb_layers[self.lod](img)
    #     x = self.conv_layers[self.lod](x)
...
class conv2d_trans(layers.Layer):
    ...
    def call(self, x):
        ...
        if self.has_gpu:
            ...
            os = [bs, h*2, width*2, out_channel]
            x = tf.transpose(x, [0, 2, 3, 1])
            y = tf.nn.conv2d_transpose(x, w, os, strides=[1,2,2,1], padding='SAME', data_format='NHWC')
...
class conv2d(layers.Layer):
    ...
    def call(self, x):
        ...
        if self.has_gpu:
            ...
            s0,s1,s2,s3 = strides
            strides = [s0,s2,s3,s1]
            y = tf.nn.conv2d(x, w, strides=strides, padding='SAME', data_format='NHWC')
            y = tf.transpose(y, [0, 3, 1, 2])
...
def blur2d(x, f=[1,2,1], normalize=True, stride=1):
    ...
    if has_gpu:
        ...
        x = tf.transpose(x, [0, 2, 3, 1])
        y = tf.nn.depthwise_conv2d(x, f, strides=[1,stride,stride,1], padding='SAME', data_format='NHWC')
        y = tf.transpose(y, [0, 3, 1, 2])
    ...
    for i in range(56):
        ...
        for var in vars:
            if var.name == dst:
                ...
                var.assign(tmp)
                break
    checkpoint_dir = './output/pretrained_stylegan/trained_model'
    checkpoint = tf.train.Checkpoint(Gen=Gen, Dis=Dis)
    manager = tf.train.CheckpointManager(checkpoint, checkpoint_dir, max_to_keep=3)
    manager.save()
    ...
    for i in range(56):
        ...
        for var in vars:
            if var.name == dst:
                ...
                var.assign(tmp)
                break
    checkpoint_dir = 'output/origin/trained_model'
    checkpoint = tf.train.Checkpoint(Gen=Gen, Dis=Dis)
    manager = tf.train.CheckpointManager(checkpoint, checkpoint_dir, max_to_keep=3)
    if manager.latest_checkpoint:
        checkpoint.restore(manager.latest_checkpoint)
...
if __name__ == '__main__':
    ...
    if ckpt:
        ...
        print(ckpt, "RESTORE FAILED")
        exit()

    Gen.mapping.lod    = int(math.log2(res) - 2)
    Gen.synthesis.lod = int(math.log2(res) - 2)
